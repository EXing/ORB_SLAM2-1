...
namespace spline {
    ...
    public:
         ...
         */
        int dersBasisFuns(double u, size_t spanIdx, int derivativeLimit, std::vector<std::vector<double>> &ders) {
            // check
            if (knotVector.empty()) {
        ...
        int evaluate(double u, int derivativeLimit,
                     std::vector<Eigen::Matrix<double, dim, 1>, Eigen::aligned_allocator<Eigen::Matrix<double, dim, 1>>> &Ders) {
            if (controlPoints.empty()) {
        ...
        const std::vector<double> &getCorrespondingUs() {
            return correspondingUs;
        }

        const std::vector<double> &getKnotVector() {
            return knotVector;
        }
